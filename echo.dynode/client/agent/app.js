...
const CONFIG = {
  //BASE_WS_URL: "wss://radiowsserver-763503917257.europe-west1.run.app/rooms/radio",
  BASE_WS_URL: "ws://localhost:7000/rooms/radio",
  AUTH_TOKEN:
  ...
  get WS_URL() {
    return `${this.BASE_WS_URL}?token=${this.AUTH_TOKEN}`;
  },
  PLAYER_WS_URL: "ws://localhost:2326",
...
const LocalWebSocketController = {
  ...
  connectAndSend() {
    ...
    Logger.log(
      `Connecting to local WebSocket at ${CONFIG.PLAYER_WS_URL}...`,
      "info"
    );

    try {
      State.localWs = new WebSocket(CONFIG.PLAYER_WS_URL);
