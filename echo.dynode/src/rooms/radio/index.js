...
export class RadioHandler extends BaseRoomHandler {
  ...
  async getBroadcastDelay(context) {
    ...
    // Prefer an explicit "delay" carried in the payload (many possible locations).
    // Accepts number or numeric string. First non-zero parsed value wins.
    const candidates = [
      context?.processed?.delay,
      context?.original?.delay,
      context?.processed?.data?.delay,
      context?.original?.data?.delay,
      context?.socket?.radioMetadata?.broadcastDelay,
      context?.socket?.radioMetadata?.metadata?.broadcastDelay,
      context?.authPayload?.metadata?.broadcastDelay,
    ];

    for (const candidate of candidates) {
      const parsedSeconds = this.parseBroadcastDelaySeconds(candidate);
      if (parsedSeconds > 0) {
        return parsedSeconds * 1000; // convert seconds -> ms
      }
    }

    // Fallback to configured env/global value
