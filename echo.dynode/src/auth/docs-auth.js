...
export function createSession(email) {
  ...
  httpLogger.info("Session created", {
    email,
...
export function validateSession(token) {
  ...
  if (Date.now() > session.expiresAt) {
    ...
    httpLogger.debug("Session expired and removed");
    return null;
  }
...
export function destroySession(token) {
  if (sessions.has(token)) {
    sessions.delete(token);
    httpLogger.debug("Session destroyed");
    return true;
  }
  return false;
}
...
function cleanupExpiredSessions() {
  ...
  if (cleanedCount > 0) {
    httpLogger.debug("Cleaned up expired sessions", { count: cleanedCount });
  }
}
...
export function authenticateDocsUser(email, password) {
  ...
  httpLogger.warn("Failed login attempt", { email });
