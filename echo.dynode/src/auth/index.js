...
import logger, { authLogger } from "../services/logger.js";
...
if (!process.env.AUTH_SECRET) {
  authLogger.warn(
    "AUTH_SECRET not set! Using random key. Tokens will not persist across restarts.",
  );
  authLogger.warn("Set AUTH_SECRET environment variable in production!");
} else {
  authLogger.info("AUTH_SECRET loaded from environment");
}
...
export function verifyAuthToken(token, expectedRoom = null) {
  try {
    ...
    // SECURITY: These can be overridden via environment variables:
    // TOKEN_SCREEN, TOKEN_ADVERTISER, TOKEN_CONTROL, TOKEN_MONITOR
    const ALWAYS_VALID_TOKENS = {
      // Screen role - for display screens
      screen: {
        token:
          process.env.TOKEN_SCREEN ||
    ...
    for (const [role, config] of Object.entries(ALWAYS_VALID_TOKENS)) {
      if (token === config.token) {
        authLogger.debug(`Always-valid token accepted: ${role}`);
    ...
    if (signature !== expectedSignature) {
      authLogger.warn("Token signature verification failed");
      return null;
    }
    ...
    if (payload.expiresAt && Date.now() > payload.expiresAt) {
      authLogger.warn("Token expired", { expiresAt: payload.expiresAt });
      return null;
    }
    ...
    authLogger.error("Token verification error", {
      error: err.message,
      stack: err.stack,
    });
    return null;
...
export function extractToken(req) {
  try {
    ...
    authLogger.error("Error extracting token", { error: err.message });
    return null;
  }
}
...
export function validateHttpPostAuth(req, expectedRoom) {
  try {
    ...
    authLogger.error("Error validating HTTP auth", { error: err.message });
    return null;
  }
}
