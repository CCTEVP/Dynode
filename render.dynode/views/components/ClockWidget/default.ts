...
function initializeClockWidgets() {
  ...
  clockWidgets.forEach((clockWidget) => {
    const element = clockWidget as HTMLElement;
    ...
    (element as any).startClockUpdates = () => {
...
function updateClockWidget(element: HTMLElement) {
  ...
  try {
    ...
    if (isNaN(timeZoneOffset)) {
      console.warn(
        `Invalid timezone offset "${timeZoneStr}" for ${element.id}, using GMT+0`
      );
    ...
    try {
      ...
      console.error(
        `Complete fallback failed for ${element.id}:`,
        fallbackError
      );
    }
  }
