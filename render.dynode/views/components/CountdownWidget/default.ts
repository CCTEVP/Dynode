...
function updateCountdownWidget(element: HTMLElement) {
  ...
  if (remainingTime <= 0) {
    ...
    if ((element as any).unsubscribeTicker) {
      (element as any).unsubscribeTicker();
    }
    return;
  }
...
function updateCountdownChildren(
  element: HTMLElement,
...
function calculateAdaptiveValues(
  ...
  const presentUnitHierarchy = unitHierarchy.filter((unit) =>
    presentUnits.includes(unit)
  );
...
function setAllChildrenToZero(element: HTMLElement) {
  const children = element.querySelectorAll("[data-name]");
  children.forEach((child) => {
...
if (window.widgetInitializer) {
  window.widgetInitializer.register(
    "CountdownWidgets",
    initializeCountdownWidgets
  );
}
